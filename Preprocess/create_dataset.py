...
noisy_speech_folder_extracted = NOISY_SPEECH_DIR
clean_speech_folder_extracted = CLEAN_SPEECH_DIR
...
def add_random_noise_to_speech_files(clean_speech_file, file_number, noise_file_number, min_length):
    ...
    sf.write(noisy_output_file, noisy_speech[:min_length * SAMPLE_RATE], SAMPLE_RATE)

    clean_output_file = os.path.join(clean_speech_folder_extracted, f'clean_speech_{file_number}.wav')
    sf.write(clean_output_file, clean_speech[:min_length * SAMPLE_RATE], SAMPLE_RATE)
...
def find_min_length(clean_speech_files):
    ...
    for file in tqdm(clean_speech_files, desc='Finding min length'):
        ...
        min_length = min(min_length, len(audio) / SAMPLE_RATE)
...
def filter_files(clean_speech_files, min_length):
    ...
    for file in tqdm(clean_speech_files, desc='Adding noise to files of minimum speech length'):
        ...
        if len(audio) / SAMPLE_RATE >= min_length:
            ...
            add_random_noise_to_speech_files(file, file_number, noise_file_number, min_length)
            file_number += 1
            noise_file_number += 1
            if file_number == DATASET_SIZE:
                break
...
if __name__ == "__main__":
    ...
    filtered_files = filter_files(clean_speech_files, DURATION)
